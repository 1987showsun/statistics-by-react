...
export default class UserEdit extends React.Component{
  constructor(props){
    ...
    this.state = {
      ...
      checkBoxSetup     : {
        ...
        selectedData          : props.seleEditData["roles"].map((item, i) => {  //初始值 { checkBoxId: [], checkBoxName:[] }
          return {
            checkBoxId          : item.id,
      ...
      formObject        : {
        ...
        roleIds           : props.seleEditData["roles"].map((item, i) => {
          return item.id
        }).toString(),
      }
    }
  ...
  checkBoxBackArray(array){
    ...
    formObject['roleIds']   = array.map((item,i)=>{return item.checkBoxId }).toString();
