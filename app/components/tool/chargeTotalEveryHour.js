...
export default class OnlineUser extends React.Component{
  ...
  checkBoxBackArray(array){
    ...
    searchFormObject['channelId']   = array.map((item,i)=>{return item.checkBoxId }).toString();
  ...
  handleSubmit(e){
    e.preventDefault();
    let searchFormObject = this.state.searchFormObject;
    //searchFormObject['channelId'] = searchFormObject['channelIds'];
    /*if(!searchFormObject.channelId.length){
      return message.warning('請选择渠道');
    } */

    this.props.dispatch( chargeTotalEveryHour(searchFormObject) );
  }
      ...
      <div className="tool">
        <form onSubmit={this.handleSubmit.bind(this)}>
          <ul className="form-list">
            <li>
              <span className="label">{lang['zh-cn']['form']['label']['searchChannel']}</span>
              <CheckBox
            ...
            <li>
              <span className="label">{lang['zh-cn']['form']['label']['searchTime']}</span>
              <div className="input-box">
                <input type="date" name="searchDate" value={this.state.searchFormObject['searchDate']} onChange={this.handleChange.bind(this)} max={this.state.maxDate}/>
              </div>
            </li>
            <li>
              <button type="submit" className="search-submit">{lang['zh-cn']['form']['btn']['search']}</button>
            </li>
          </ul>
        </form>
      </div>
